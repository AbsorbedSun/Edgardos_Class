...
/*
void quickSort(int *arregloDes, int p, int r)
...
void quickSort(int *arregloDes, int p, int r) {
    if(p < r){
        int j = Pivot(arregloDes, p, r);
        quickSort(arregloDes, p, j-1);
        quickSort(arregloDes, j+1, r);
    }
}

/*
void Pivot(int *arregloDes, int p, int r)
Recibe: int * arreglo como arregloDes y n como tamaño del arreglo
...
algoritmo de ordenamiento Merge o mezcla ()
*/
int Pivot(int *arregloDes, int p, int r) {
    int piv = arregloDes[p], i = p + 1, j = r;
    while (i <= j){
        while (arregloDes[i] <= piv && i <= r){
            i++;  
        }
    ...
    Intercambio(arregloDes, p, j);
    return j;
}

/*
void Intercambio(int *arregloDes, int i, int j)
Recibe: int * arreglo como arregloDes y n como tamaño del arreglo
